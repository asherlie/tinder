...
#                                               print(pic)
        ...
        def get_user_list(self, stop_at=-1):
            ...
            while True:
                tmp = self.get_recs()
                if 'message' in tmp.json() and tmp.json()['message'] == 'recs_exhausted':
                    msg = 'recs exhausted'
                    break
                if tmp.status_code != 200 or 'results' not in tmp.json():
                    msg = 'vague bad results'
                    break
                if stop_at != -1 and len(ll) >= stop_at:
                    msg = 'stopped at cut off param'
                    break
                ll.append(tmp.json()['results'])
                print('currently scraped ' + str(len(ll)) + ' user clumps')

            print('user list scraped with message: ' + msg)
        ...
        def update_user_list(self):
            ...
            if ul == []:
                print('get_user_list() returned an empty list. if this is the only error you\'re getting, something has gone terribly wrong.')
                return old_users
